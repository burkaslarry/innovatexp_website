...
import { getAiEraQualityBundle } from "@/content/pillar-bundles";
...
export async function generateMetadata({
  ...
  const b = getAiEraQualityBundle(locale);
...
export default async function AiEraQualityPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const b = getAiEraQualityBundle(loc);
  ...
  const bookLabel = localeUsesChineseCopy(loc) ? "預約諮詢" : "Book a consultation";
    ...
    <main className="min-h-screen bg-white dark:bg-gray-900">
      ...
      <div className="container mx-auto max-w-4xl px-4 py-16">
        ...
        <section className="rounded-xl border border-slate-200 bg-slate-50 p-6 dark:border-gray-700 dark:bg-gray-800">
          <p className="mb-4 text-gray-700 dark:text-gray-300">{b.metaDescription}</p>
          <Link
            href={`/${locale}/bookme`}
